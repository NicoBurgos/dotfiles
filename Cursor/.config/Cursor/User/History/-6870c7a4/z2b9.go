...
type TransactionInput struct {
	...
	Date     string  `json:"date"` // string del frontend
}
...
func getTransactions(c *fiber.Ctx) error {
	rows, err := db.Queryx("SELECT * FROM transactions ORDER BY date DESC")
	if err != nil {
		return c.Status(500).SendString("DB error")
	}
	defer rows.Close()
	...
	// Si no hay transacciones, devolver array vacío en lugar de null
	if transactions == nil {
...
func getPortfolio(c *fiber.Ctx) error {
	...
	// Si no hay holdings, devolver array vacío en lugar de null
	if holdings == nil {
		holdings = []Holding{}
	}
