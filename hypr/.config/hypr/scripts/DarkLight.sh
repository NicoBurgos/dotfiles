...
for pid in waybar rofi swaync ags swaybg; do
  killall -SIGUSR1 "$pid" 2>/dev/null
done
...
# Set awww options
awww="awww img"
AWWW_PARAMS="--transition-bezier .43,1.19,1,.4 --transition-fps 60 --transition-type grow --transition-pos 0.925,0.977 --transition-duration 2"
...
for pid_kitty in $(pidof kitty); do
  kill -SIGUSR1 "$pid_kitty" 2>/dev/null
done
...
$awww "${next_wallpaper}" $AWWW_PARAMS
...
if [ "$next_mode" = "Dark" ]; then
  ...
  sed -i '/^background:/s/.*-i/background: rgba(255,255,255,0.9);/' $wallust_rofi
fi
